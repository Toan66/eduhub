...
const TestEditor = () => {
    ...
    const [testData, setTestData] = useState({ testTitle: '', testDescription: '', questions: [] });
    ...
    useEffect(() => {
        const fetchTest = async () => {
            try {
                const response = await axios.get(`https://localhost:7291/api/Test/${testId}/details`);
    ...
    const handleQuestionChange = (e, questionId) => {
        const newQuestions = questions.map(question =>
            question.questionId === questionId ? { ...question, questionText: e.target.value } : question
        );
        setQuestions(newQuestions);
    };
        ...
        const newQuestions = questions.map(question => {
            if (question.questionId === questionId) {
                const newAnswers = question.answers.map(answer =>
                    answer.answerId === answerId ? { ...answer, answerText: e.target.value } : answer
                );
    ...
    const handleRemoveAnswer = (questionId, answerId) => {
        const newQuestions = questions.map(question => {
            if (question.questionId === questionId) {
                const newAnswers = question.answers.filter(answer => answer.answerId !== answerId);
                return { ...question, answers: newAnswers };
            }
            return question;
        });
    ...
    const handleOnDragEndAnswers = (result) => {
        ...
        const questionIndex = questions.findIndex(q => q.questionId.toString() === questionId);
    ...
    return (
        <div className="container mx-auto max-w-screen-lg">
            ...
            <div className='w-full lg:max-w-screen-sm'>
                ...
                <div className="mb-4">
                    <label htmlFor="testTitle" className="block text-gray-700 text-sm font-bold mb-2">Test Title</label>
                    <input type="text" id="testTitle" value={testTitle} onChange={(e) => setTestTitle(e.target.value)} className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required />
                </div>

                <div className="mb-4">
                    <label htmlFor="testDescription" className="block text-gray-700 text-sm font-bold mb-2">Test Description</label>
                    <textarea type="text" id="testDescription" value={testDescription} onChange={(e) => setTestDescription(e.target.value)} className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required />
                </div>
