...
function CourseEditor() {
		...
		const fetchCourse = async () => {
			try {
				const response = await axios.get(
					`https://localhost:7291/api/Course/${courseId}/details/edit`,
	...
	const handleUpdateTitle = async () => {
		try {
			await axios.put(
				`https://localhost:7291/api/Course/${courseId}/updateName`,
				{
					courseName: newTitle,
				},
				{ withCredentials: true }
	...
	const handleUpdateDescription = async () => {
		try {
			await axios.put(
				`https://localhost:7291/api/Course/${courseId}/updateDescription`,
				{
	...
	const updateCourseImage = async (imageUrl) => {
		try {
			await axios.put(
				`https://localhost:7291/api/Course/${courseId}/updateImage`,
				{
	...
	const handleUpdateCategory = async () => {
		try {
			await axios.put(
				`https://localhost:7291/api/Course/${courseId}/updateCategory`,
				{
	...
	const handleUpdateLevel = async () => {
		try {
			await axios.put(
				`https://localhost:7291/api/Course/${courseId}/updateLevel`,
				{
	...
	const handleUpdatePrice = async () => {
		try {
			await axios.put(
				`https://localhost:7291/api/Course/${courseId}/updatePrice`,
				{
	...
	const handleUpdateEarnings = async () => {
		try {
			await axios.put(
				`https://localhost:7291/api/Course/${courseId}/updateEarn`,
				{
	...
	async function handleOnDragEnd(result) {
		...
		try {
			const response = await axios.put(
				`https://localhost:7291/api/Chapter/Course/${courseId}/updateChapterOrder`,
				{
	...
	return (
		<div className="container mx-auto px-4 sm:max-w-screen-lg">
			<div className="text-3xl font-bold mb-4 flex align-middle items-center justify-between">
				Course Setup
				<button
					className=" text-xl bg-gray-800 text-white font-semibold px-5 py-2 rounded-md mt-5 float-right"
				...
				<div className="mb-8 w-full lg:w-1/2 pr-3">
					...
					<div className="rounded-lg bg-indigo-50 p-3 mb-6">
						...
						{editTitle ? (
							...
							<div className="font-normal">{course.courseName}</div>
						)}
					</div>
					...
					<div className="rounded-lg bg-indigo-50 p-3 mb-6">
						<div className="flex flex-row justify-between mb-4 text-lg">
							...
							<button
								onClick={() => {
									...
									setNewDescription(course.courseDescription);
								}}
						...
						{editDescription ? (
							<div>
								<textarea
									className="w-full p-3 h-32 rounded-md"
							...
							<div className="font-normal">{course.courseDescription}</div>
						)}
					</div>
				...
				<div className="mb-8 w-full lg:w-1/2 pl-3">
					...
					<div className="rounded-lg bg-indigo-50 p-3 mb-6">
						<div className="flex flex-row justify-between items-center mb-4 text-lg">
							...
							<Link
								to={`/Course/${course.courseId}/Chapter/Create`}
								className="font-semibold w-auto text-right items-center"
							>
								<span className="inline-block mr-2">
					...
					<div className="rounded-lg bg-indigo-50 p-3 mb-6">
						...
						{editCategory ? (
							...
							<div className="font-normal">
								{categories.find(
									(category) => category.courseCategoryId === course.categoryId
								)?.courseCategoryName || "Category not found"}
							</div>
						)}
					</div>
					...
					<div className="rounded-lg bg-indigo-50 p-3 mb-6">
						<div className="flex flex-row justify-between mb-4 text-lg">
							...
							<button
								onClick={() => {
									setEditPrice(true);
									setNewPrice(course.coursePrice);
								}}
								className="font-semibold w-auto text-right items-center"
							>
								<span className="inline-block mr-2">
